...
class Food:
    ...
    def create_special_food(self):
        self.food.append(Turtle(visible=False))
        self.food[1].shape('square')
        self.food[1].penup()
        self.food[1].color('red')
        self.food[1].shapesize(FOOD_SIZE)
        self.food[1].goto(random.randint(2*-DISTANCE, 2*DISTANCE), random.randint(2*-DISTANCE, 2*DISTANCE))


    def special_food_unavailable(self):
        self.food[1].hideturtle()
        self.food[1].goto(2*-DISTANCE, 2*DISTANCE)
